...
int main(void)
{
	...
	tuple_t *about = 
		...
		(
			3,
			(object_t) { .type = CONST_STRING_T, .cs = "math_python" },
			(object_t) { .type = CONST_STRING_T, .cs = "1.0.0.1" },
	...
	while (true)
	{
		...
		{
			case 0:
				fputs(u8"Введите трёхзначное число: ", stdout);
				number = read_int64();
...
static list_t *check(long long number)
{
	if (number < 100 || number > 999)
		...
		(
			...
			{
				.type = STRING_T,
				.s = u8"В числе есть одинаковые цифры"
			}
		);
	else
	{
		list_t *num_list = calc(number);
		list_t *str_list = get_str_num(num_list);
		list_clear(&num_list);
		return str_list;
	}
}
